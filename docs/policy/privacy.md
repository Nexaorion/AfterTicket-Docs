...
AfterTicket Team 委托 **Nexaorion**（以中国香港个人身份注册的 Stripe 收款主体）代为处理付费计划的收款、订阅管理和争议解决。Nexaorion 按照 AfterTicket Team 的指示处理相关支付数据，其处理活动还可能受中国香港《个人资料（私隐）条例》（PDPO）约束。
...
- 付费计划、订阅和通过 Stripe 处理的支付；
- 与支持、安全、隐私、版权投诉有关的沟通。

本政策不适用于 Bilibili、活动方、Stripe 自身数据处理活动、支付机构、Sentry、用户自行选择的推送服务或其他独立第三方的自身数据处理活动。Stripe 按其自身隐私政策独立处理的数据，不在本政策范围内。
...
- 付费计划通过 Stripe 处理支付，Nexaorion（中国香港）作为 Stripe 收款主体代为收款和管理订阅；
- 我们为每个用户设置集控节点接入上限，付费计划用户可获得更高配额和专用集控服务器；
- Control/Dashboard 支持 KYC 实名认证功能，认证服务由 **Didit** 提供；认证过程会收集浏览器设备指纹、IP 地址、证件信息（含照片）及活体识别视频，用于核身和合规；认证材料将在提交后 6 个月时自动永久删除，此后不可恢复；
- 系统会根据 Bilibili UID 执行风险控制，存在异常的 UID 可能受到访问限制或功能禁用。
...
- Bilibili UID 风控评估结果、风控标签和访问限制记录。
...
当您购买、管理或使用 AfterTicket 付费计划时，我们可能处理：

- Stripe Customer ID、Subscription ID 和 Payment Intent ID；
- 付费计划类型、状态、开始日期、续订日期、到期日期和取消日期；
- 支付方式类型和最后四位标识；
- 账单金额、货币、发票、收据和支付状态；
- 集控节点配额、已用节点数和计划权益；
- 退款、争议和退单记录；
- 您提供给 Stripe 的账单姓名、账单地址和电子邮箱。

完整的银行卡号、CVV 和银行账户凭证由 Stripe 直接处理，不经过我们的服务器。Nexaorion 作为 Stripe 收款主体可能在处理支付、订阅管理和争议解决过程中访问上述部分数据。
...
KYC 实名认证由 **Didit** 身份认证服务提供。当您完成 KYC 认证时，Didit 在认证流程中收集，我们可能收到或处理：
...
- 认证流程中的 IP 地址；
- 认证提交时间、认证结果和认证状态记录；
- Didit 为完成核身所需的其他辅助信息。

**实名认证材料（包括证件照片、扫描件及原始证件信息）将在提交后满 6 个月时自动永久删除，该操作不可撤销。** 认证状态记录（已通过认证的标识）可能在材料删除后继续保留。
...
- Stripe 和 Nexaorion 提供的支付、订阅和争议信息；
...
- 处理付费计划的订阅、计费、续订、降级、取消和退款；
- 管理和执行集控节点接入配额；
- 处理支付争议和退单；
- 提供支持、排查故障、分析性能和修复错误；
- 防止滥用、欺诈、未经授权访问和安全事件；
- 根据 Bilibili UID 执行风险控制评估，识别并限制存在异常行为的账号；
- 通过 Didit 身份认证服务收集、核实并保存 KYC 实名认证材料（含浏览器设备指纹、IP 地址、证件信息及活体识别视频），用于核身和合规；并在 6 个月后自动永久删除认证原始材料；
- 执行协议、可接受使用政策和知识产权政策；
- 遵守法律义务、响应合法请求和处理争议；
- 改进服务并评估功能使用情况。
...
### 8.2 Stripe 和 Nexaorion

为处理付费计划的支付、订阅、退款和争议，我们会将必要的支付和订阅数据发送至 Stripe。Nexaorion（以中国香港个人身份注册的 Stripe 收款主体）受 AfterTicket Team 委托，作为收款方接收和处理支付数据。

Stripe 按其自身隐私政策和服务协议独立处理的数据，不在我们的控制范围内。Nexaorion 对支付数据的处理还受中国香港《个人资料（私隐）条例》约束。
...
KYC 实名认证由 **Didit** 提供服务。在认证流程中，浏览器设备指纹、IP 地址、证件信息（含照片）及活体识别视频将通过 Didit 的服务进行处理，用于核身和满足合规要求。Didit 按其自身隐私政策和服务协议独立处理上述数据，其处理活动不在本政策控制范围内。
...
- KYC 认证数据通过 Didit 处理，可能在新加坡境外地点进行；
- 支付数据通过 Stripe 在美国或其他地点处理；
- Nexaorion 在中国香港处理收款和订阅管理数据；
- 您从其他国家或地区访问官方服务；
- Bilibili、托管、更新、推送或其他服务在境外处理数据；
- 您选择位于境外的节点、Master 或推送服务。

在我们负责的范围内，我们会采取合理措施，要求境外接收方对个人数据提供与新加坡 PDPA 要求相当的保护，或者依赖适用法律允许的其他跨境传输方式。Nexaorion 在中国香港的数据处理同时受中国香港《个人资料（私隐）条例》约束。
...
- **KYC 实名认证材料（含证件照片、扫描件及原始证件信息）将在提交后满 6 个月时自动永久删除，该操作不可撤销且不可恢复**；认证状态记录（已认证标识）可能在材料删除后继续保留；
...
- 付费计划的支付记录、发票和订阅历史可能按照税务、会计、争议处理和审计要求保留适用法律规定的期限；
- 本地配置、会话和日志保留在您的设备上，直至您自行删除；
- Sentry、托管服务商、备份和第三方推送服务按其配置和政策保留数据；
- Stripe 按其数据保留政策独立保留支付数据；
...
AfterTicket 可能包含或连接 Bilibili、Stripe、Sentry、推送服务、支付页面和其他第三方。我们不控制这些第三方的隐私和安全实践。您应阅读其政策，并仅向可信接收方提交数据。
